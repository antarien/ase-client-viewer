 ...
 * @brief       AdwPreferencesWindow builder for ViewerSettings
 * @description Uses the libadwaita C API directly because adwaitamm (C++
 *              bindings) is not widely packaged. Each row mutates the
 *              settings struct, persists via save_settings(), and invokes
 *              the on_changed slot so the main canvas can redraw.
 *
...
GtkWidget* build_appearance_page(PrefContext* ctx) {
    ...
    g_signal_connect(scheme_row, "notify::selected",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            ...
            c->settings->color_scheme = static_cast<uint8_t>(
                adw_combo_row_get_selected(ADW_COMBO_ROW(obj)));
            save_and_notify(c);
        }), ctx);
    ...
    g_signal_connect(font_row, "notify::value",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
            c->settings->font_size = static_cast<int>(
                adw_spin_row_get_value(ADW_SPIN_ROW(obj)));
            save_and_notify(c);
        }), ctx);
    ...
    g_signal_connect(code_font_row, "notify::selected",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
...
GtkWidget* build_documents_page(PrefContext* ctx) {
    ...
    g_signal_connect(mode_row, "notify::selected",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
    ...
    g_signal_connect(lang_row, "notify::selected",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
    ...
    g_signal_connect(reload_row, "notify::active",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
...
GtkWidget* build_viewer_page(PrefContext* ctx) {
    ...
    g_signal_connect(ln_row, "notify::active",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
    ...
    g_signal_connect(math_row, "notify::active",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
    ...
    g_signal_connect(mmrd_row, "notify::active",
        G_CALLBACK(+[](GObject* obj, GParamSpec*, gpointer data) {
            auto* c = static_cast<PrefContext*>(data);
...
void show_preferences_window(
    ...
    sigc::slot<void()> on_changed)
{
    // Static context - lives as long as the prefs window itself. Only
    // one preferences window is allowed at a time.
    static PrefContext ctx;
    ctx.settings   = &settings;
    ctx.config_dir = config_dir;
    ctx.on_changed = std::move(on_changed);
